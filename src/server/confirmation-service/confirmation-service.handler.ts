...
export class ConfirmationServiceHandler {
  ...
  private checkStatus(_data: ConfirmationData): boolean {
    if (_data.status === ConfirmationStatus.Created) {
      _data.error = new Exception(ConfirmationErrors.NotSent, ConfirmationErrorsMessages[ConfirmationErrors.NotSent], {
    ...
    if (_data.status === ConfirmationStatus.Confirmed) {
      _data.error = new Exception(ConfirmationErrors.Used, ConfirmationErrorsMessages[ConfirmationErrors.Used]);
  ...
  private checkLifetime(_data: ConfirmationData): boolean {
    if (_data.sentAt && _data.sentAt + 60000 > Date.now()) {
      _data.error = new Exception(ConfirmationErrors.TooManyRequest, ConfirmationErrorsMessages[ConfirmationErrors.TooManyRequest]);
  ...
  private checkExpires(_data: ConfirmationData): boolean {
    if (_data.expires && _data.expires < Date.now()) {
      _data.error = new Exception(ConfirmationErrors.Expired, ConfirmationErrorsMessages[ConfirmationErrors.Expired]);
  ...
  private checkAttempts(_data: ConfirmationData): boolean {
    if (!_data.counter) {
      _data.error = new Exception(ConfirmationErrors.Deactivated, ConfirmationErrorsMessages[ConfirmationErrors.Deactivated], {
        attempts: _data.counter,
      });
  ...
  private checkEqual(_data: ConfirmationData, _code: string): boolean {
    if (_data.code && !compareSync(String(_code), _data.code)) {
      _data.error = new Exception(ConfirmationErrors.Incorrect, ConfirmationErrorsMessages[ConfirmationErrors.Incorrect], {
        attempts: _data.counter,
      });
