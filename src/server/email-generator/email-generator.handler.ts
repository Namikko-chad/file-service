...
export class EmailGeneratorHandler {
  ...
  public buildTemplate<T extends EmailType>(key: EmailType, fields: FieldsEmailRequest[T]): EmailStub {
    ...
    const file = fs.readFileSync(path.resolve(this.templatesDir, pattern.path + '.handlebars'), 'utf-8');
