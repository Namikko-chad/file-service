...
import { IBoomData,IOutputEmpty, IOutputOk, IOutputPagination, } from '../interfaces';
...
export function error<T>(code: number, msg: string, data: T): Boom<IBoomData<T>> {
  return new Boom(msg, {
    data: {
      code,
      data,
...
export function handleValidationError(request: Request, h: ResponseToolkit, err: Error | undefined): Boom {
  request;
  h;
